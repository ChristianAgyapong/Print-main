        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    style: 'destructive',
                    onPress: async () => {
                        const success = await storageService.deleteUpload(upload.id, upload.file_url);
                        if (success) {
    ...
    const renderUpload = ({ item }: { item: Upload }) => (
        <TouchableOpacity style={styles.uploadCard} activeOpacity={0.7}>
            <View style={styles.uploadIcon}>
                <Ionicons name={getFileIcon(item.file_type) as any} size={32} color="#3B82F6" />
            </View>
            <View style={styles.uploadInfo}>
                ...
                <Text style={styles.uploadMeta}>
                    {storageService.formatFileSize(item.file_size)} • {formatDate(item.created_at)}
                </Text>
            </View>
            <TouchableOpacity
                ...
                <>
                    <View style={styles.statsBar}>
                        <Text style={styles.statsText}>
                            {uploads.length} {uploads.length === 1 ? 'design' : 'designs'} saved
                        </Text>
                    </View>
                    <FlatList
                        data={uploads}
