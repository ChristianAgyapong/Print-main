  ...
  async uploadFile(
    ...
    try {
      ...
      if (!ALLOWED_TYPES.includes(fileType)) {
        throw new Error(
          "File type not supported. Please upload PDF, JPG, PNG, AI, or PSD files.",
        );
      }

      // Generate unique filename
      const fileNameWithExt = (file as any).name || "file";
      const fileExt = fileNameWithExt.split(".").pop() || "file";
  ...
  async uploadAvatar(
    ...
    try {
      ...
      try {
        ...
        throw new Error(
          `Failed to read image: ${fileError.message}. Please try selecting the image again.`,
        );
      }
      ...
      if (arrayBuffer.byteLength > MAX_AVATAR_SIZE) {
        throw new Error(
          `Image size exceeds ${MAX_AVATAR_SIZE / 1024 / 1024}MB limit`,
        );
      }
      ...
      if (error.message.includes("Network request failed")) {
        ...
        throw new Error(
          "Could not load image. Please try selecting the image again.",
        );
      }
      throw error;
    }
